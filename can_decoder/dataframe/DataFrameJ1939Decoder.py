...
class DataFrameJ1939Decoder(DataFrameDecoder):
        ...
        for unique_id in unique_multiplexed_ids:
            ...
            for signal in signals:
                if signal.is_multiplexer:
                    ...
                    self._decode(
                        signal=signal, signal_data=signal_data, signal_index=signal_index, signal_ids=signal_ids,
                        frame=frame, ignore_invalid=ignore_invalid, frame_name=frame_name
                        )
    ...
    def _decode(self, signal, signal_data, signal_index, signal_ids, frame: Frame, ignore_invalid: bool,
                frame_name=""):
        ...
        if ignore_invalid and not signal.is_signed and not signal.unit == "char":
            limit = get_j1939_limit(signal.size)
            valid_indices = np.argwhere(signal_data_raw < limit)[:, 0]

        if valid_indices.shape[0] == 0 or signal_data_raw.size == 0:
